    ...
    <div class="dashboard-page-chart__body">
      <b-row class="dashboardBody">
        <b-col lg="12" xl="12">
          ...
          <template v-for="(data,index) in ModelArchived">
...
export default {
  ...
  methods: {
    ...
    rowClicked(ev, id) {
      console.log(ev, id)
      let param = ev;
      param.project_id = this.ModelArchived[id].id;
    ...
    getHeader() {
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/management-detail/header/sales-lead', param).then((response) => {
        ...
        for (let i = 0; i < data.length; i++) {
          this.ModelArchived.push({
            ...data[i],
            propList: {
              url: "/api/hoonian-website/dashboard/operation-detail/sales-lead/archived-list",
    ...
    renderList() {
      for (let i = 0; i < this.ModelArchived.length; i++) {
        this.ModelArchived[i].propList.ParamWhere = this.search;
        this.$refs['ref_archived_'+i][0].doGetList("");
      }
    }
  },
  mounted() {
