    ...
    <div class="dashboard-page-chart__body">
      <b-row class="dashboardBody">
        <b-col lg="12" xl="12">
          ...
          <template v-for="(data,index) in ModelUnattended">
                ...
                >
                    ...
                    <template slot="ToolbarTable">
                      ...
                      <b-col md="1" lg="1" xl="1" align-self="center" class="font-poppins noPadding">
                        <div align-self="center" style="background: linear-gradient(180deg, #FF0045 0%, #8000FF 100%);" class="badge-header-list">
...
export default {
  ...
  methods: {
    ...
    rowClicked(ev, id) {
      console.log(ev, id)
      let param = ev;
      param.project_id = this.ModelUnattended[id].id;
      this.$store.commit("setParamPage", param);
      this.$router.push({ name: "OP_SalesLeadUnattendedForm" });
    },
    doDeleteClick(ev, id) {
      console.log(ev, id)
    },
    ...
    getHeader() {
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/operation-detail/sales-lead/unattended-header', param).then((response) => {
        if (response == null) return;
        let data = response.data;
        this.ModelUnattended = [];
        for (let i = 0; i < data.length; i++) {
          this.ModelUnattended.push({
            ...data[i],
            propList: {
              url: "/api/hoonian-website/dashboard/operation-detail/sales-lead/unattended-list",
    ...
    renderList() {
      for (let i = 0; i < this.ModelUnattended.length; i++) {
        this.ModelUnattended[i].propList.ParamWhere = this.search;
        this.$refs['ref_unattended_'+i][0].doGetList("");
      }
    }
  },
  mounted() {
