              ...
              <b-col lg="12" xl="12">
                ...
                >
                  ...
                  <!-- <template slot="ToolbarTable">
                    <b-col style="margin-right: 10px; font-size: 14px; color: rgba(0, 0, 0, 0, 0.5); text-shadow: 0.5px 0px; letter-spacing: -0.5px;" align-self="center" class="col-right font-poppins">
                      <b>
                        Total Sales Lead
                      </b>
                    </b-col>
                    ...
                    >
                      <span style="margin: auto; font-size: 14px;">
                      {{data.total_transaction == 0 || data.total_sales_lead == 0 ?
                        0 :
                        (data.total_transaction / data.total_sales_lead) * 100}}
                      </span>
                    </b-col>
                  </template> -->
                  <template slot="assigned_date" slot-scope="data">
                    {{momentUnix(data.item.assigned_date, "DD MMM YYYY")}}
                  </template>
                  <template slot="last_update" slot-scope="data">
                    {{momentUnix(data.item.last_update, "DD MMM YYYY")}}
                  </template>
                </HOOList>
              </b-col>
            </b-row>
...
export default {
  ...
  methods: {
    ...
    rowClicked(ev, id) {
      console.log(ev, id)
      let param = ev;
      param.project_id = this.ModelSalesLead[id].id;
      param.sales_lead_id = param.sales_lead_id;
      this.$store.commit("setParamPage", param);
      this.$router.push({ name: "OP_SalesLeadLogbook" });
    },
    doDeleteClick(ev, id) {
      console.log(ev, id)
    },
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/management-detail/header/sales-lead', param).then((response) => {
        ...
        for (let i = 0; i < data.length; i++) {
          this.ModelSalesLead.push({
            ...data[i],
            propList: {
              url: "/api/hoonian-website/dashboard/operation-detail/sales-lead/total-list",
