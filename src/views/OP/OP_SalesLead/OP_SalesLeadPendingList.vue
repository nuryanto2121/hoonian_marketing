    ...
    <div class="dashboard-page-chart__body">
      <b-row class="dashboardBody">
        <b-col lg="12" xl="12">
          ...
          <template v-for="(data,index) in ModelPending">
...
export default {
  ...
  methods: {
    ...
    rowClicked(ev, id) {
      console.log(ev, id)
      let param = ev;
      param.project_id = this.ModelPending[id].id;
      this.$store.commit("setParamPage", param);
      this.$router.push({ name: "OP_SalesLeadPendingForm" });
    },
    doDeleteClick(ev, id) {
      console.log(ev, id)
    },
    ...
    getHeader() {
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/management-detail/header/sales-lead', param).then((response) => {
        ...
        for (let i = 0; i < data.length; i++) {
          this.ModelPending.push({
            ...data[i],
            propList: {
              url: "/api/hoonian-website/dashboard/operation-detail/sales-lead/pending-list",
    ...
    renderList() {
      for (let i = 0; i < this.ModelPending.length; i++) {
        this.ModelPending[i].propList.ParamWhere = this.search;
        this.$refs['ref_pending_'+i][0].doGetList("");
      }
    }
  },
  mounted() {
