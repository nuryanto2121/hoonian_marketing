    ...
    <div class="dashboard-page-chart__body">
      <b-row class="dashboardBody">
        <b-col lg="12" xl="12">
          ...
          <template v-for="(data,index) in ModelBooking">
              ...
              <b-col lg="12" xl="12">
                ...
                >
                    <template slot="TitleTable">
                      <b-col lg="10" xl="10" style="padding-left: unset !important;" class="title-list-primary text-single">
                        {{data.project_name}}
                      </b-col>
                    </template>
                    <template slot="ToolbarTable">
                      <b-col md="2" lg="2" xl="2" align-self="center" class="font-poppins noPadding" style="text-align: right;">
                        <ABSButton
                          :text="`${data.overdue} \xa0\xa0\xa0\xa0\xa0 Overdue`"
                    ...
                    <template slot="tower" slot-scope="data">
                        <span :style="data.item.status == 1 ? 'color: red;' : data.item.status == 2? 'color: grey' : ''">
                        {{ data.item.tower }}
                        </span>          
                    </template>
                    ...
                    <template slot="principle" slot-scope="data">
                      <span :style="data.item.status == 1? 'color: red;': data.item.status == 2? 'color: grey': ''">
                        {{ data.item.principle }}
                      </span>
                    </template>
...
export default {
  ...
  methods: {
    ...
    rowClicked(ev, id) {
      console.log(ev, id)
      let param = ev;
      param.project_id = this.ModelBooking[id].id;
    ...
    getHeader() {
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/operation-detail/booking/header', param).then((response) => {
        if (response == null) return;
        let data = response.data;
        this.ModelBooking = [];
        for (let i = 0; i < data.length; i++) {
          this.ModelBooking.push({
            ...data[i],
            propList: {
              url: "/api/hoonian-website/dashboard/operation-detail/booking/grid",
    ...
    renderList() {
      for (let i = 0; i < this.ModelBooking.length; i++) {
        this.ModelBooking[i].propList.ParamWhere = this.search;
        this.$refs['ref_list_'+i][0].doGetList("");
      }
    }
  },
  mounted() {
