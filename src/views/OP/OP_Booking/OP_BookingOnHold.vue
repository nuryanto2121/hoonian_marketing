              ...
              <b-col lg="12" xl="12">
                ...
                >
                  ...
                  <template slot="hours" slot-scope="data">
                    {{ isCurrency(unixTimeDifference(momentDateToUnix(momentDateFormatting(new Date(), 'YYYY-MM-DD HH:mm:ss'), 'YYYY-MM-DD HH:mm:ss'), data.item.hours).hours, 0) }}
                  </template>
                </HOOList>
              </b-col>
            </b-row>
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/operation-detail/booking/header', param).then((response) => {
        ...
        for (let i = 0; i < data.length; i++) {
          this.ModelBooking.push({
            ...data[i],
            propList: {
              url: "/api/hoonian-website/dashboard/operation-detail/booking/hold-grid",
