...
export default {
  data() {
    return {
      propList: {
        url: '/api/hoonian-website/view-tower-cluster-list',
        initialWhere: '',
        SortField: '',
        SortBy: '',
        ParamWhere: '',
  ...
  methods: {
    ...
    doViewClick(record, index) {
      ...
      param.ForCluster = record;
      param.ForCluster.isEdit = false;
      param.ForCluster.isView = true;
      this.$store.commit("setParamPage", param);
      this.$router.push({ name: "OP_TowerClusterView" });
    },
