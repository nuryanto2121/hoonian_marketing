    ...
    <div class="dashboard-page-chart__body">
      <b-row class="dashboardBody">
        <b-col lg="12" xl="12">
          ...
          <template v-for="(data,index) in ModelTickets">
              ...
              <b-col lg="12" xl="12">
                ...
                >
                  ...
                  <template slot="ticket_date" slot-scope="data">
                    {{momentUnix(data.item.ticket_date, "DD MMM YYYY")}}
                  </template>
                </HOOList>
              </b-col>
            </b-row>
...
export default {
  ...
  methods: {
    ...
    rowClicked(ev, id) {
      var param = ev;
      param.project_id = this.ModelTickets[id].id;
    ...
    getHeader() {
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/management-detail/header/ticket', param).then((response) => {
        if (response == null) return;
        let data = response.data;
        this.ModelTickets = [];
        for (let i = 0; i < data.length; i++) {
          this.ModelTickets.push({
            ...data[i],
            propList: {
              url: "/api/hoonian-website/dashboard/management-detail/grid/ticket",
    ...
    renderList() {
      for (let i = 0; i < this.ModelTickets.length; i++) {
        this.ModelTickets[i].propList.ParamWhere = this.search;
        this.$refs['ref_tickets_'+i][0].doGetList("");
      }
    }
  },
  mounted() {
