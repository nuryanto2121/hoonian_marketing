                  ...
                  <div class="card">
                    ...
                    <b-row class="title-primary">
                        <b-col style="max-width: max-content !important;">
                            <span style="font-size: 18px; color: #4a93b3;">Waiting List Detail</span>
                        </b-col>
                      ...
                      >
                        ...
                        <template slot="ToolbarTable">
                          <b-col align-self="center" class="col-right font-poppins lbl-header-list">
                            <b>
                              Total Waiting Buyers
                            </b>
                          </b-col>
                          <b-col md="1" lg="1" xl="1" align-self="center" class="font-poppins noPadding">
                            <div align-self="center" style="--bgcolor: rgb(168 225 12);" class="badge-header-list">
                        ...
                        <template slot="status" slot-scope="data">
                          <div :style="data.item.status == 'Reserved' ? 'background-color: red; color: white;' : ''">
                            {{data.item.status}}
                          </div>
                        </template>
                        ...
                        <template slot="trans_date" slot-scope="data">
                          {{momentUnix(data.item['trans_date'], "DD MMM YYYY")}}
                        </template>
                      </HOOList>
                  </b-col>
              </b-row>
...
export default {
  ...
  methods: {
    ...
    rowClicked(ev, id) {
      if (ev.status !== 'Reserved') return
      let param = ev;
      param.isEdit = true;
      this.$store.commit("setParamPage", param);
      this.$router.push({ name: "OP_WaitingListForm" });
    },
    doDeleteClick(ev, id) {
      console.log(ev, id)
    },
    ...
    getProject() {
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/management-detail/reservation/header/waiting-list', param).then((response) => {
        ...
        for (let i = 0; i < data.length; i++) {
          this.ModelProject.push({
            ...data[i],
            propList: {
              url: "/api/hoonian-website/dashboard/management-detail/reservation/grid/waiting-list",
    ...
    renderList(id = -1) {
      if (id < 0) {
        ...
        this.$refs['ref_waiting_list_'+id][0].doGetList("");
      }
    }
  },
  mounted() {
