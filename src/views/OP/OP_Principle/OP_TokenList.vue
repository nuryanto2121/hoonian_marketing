                ...
                >
                    ...
                    <template slot="ToolbarTable">
                        ...
                        <b-col md="1" lg="1" xl="1" align-self="center" class="font-poppins noPadding">
                            <div align-self="center" style="--bgcolor: #ffc700;" class="badge-header-list">
...
export default {
  ...
  methods: {
    ...
    rowClicked(ev, id) {
      console.log(ev, id)
      let param = ev;
      param.project_id = this.ModelToken[id].id;
      this.$store.commit("setParamPage", param);
      this.$router.push({ name: "OP_TokenForm" });
    },
    doDeleteClick(ev, id) {
      console.log(ev, id)
    },
    ...
    getHeader() {
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/operation-detail/token/header', param).then((response) => {
        ...
        for (let i = 0; i < data.length; i++) {
          this.ModelToken.push({
            ...data[i],
            propList: {
              url: "/api/hoonian-website/dashboard/operation-detail/token/grid",
    ...
    renderList() {
      for (let i = 0; i < this.ModelToken.length; i++) {
        ...
        this.$refs['ref_token'+i][0].doGetList("");
      }
    }
  },
  mounted() {
