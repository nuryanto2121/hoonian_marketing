    ...
    <div class="dashboard-page-chart__body">
      <b-row class="dashboardBody">
        <b-col lg="12" xl="12">
          ...
          <template v-for="(data,index) in ModelBlockedPrinciple">
            <b-row v-bind:key="index">
              <b-col lg="12" xl="12">
                <HOOList
                  ...
                  :ref="'ref_blocked_principle'+index"
...
export default {
  ...
  methods: {
    ...
    rowClicked(ev, id) {
      console.log(ev, id)
      let param = ev;
      param.project_id = this.ModelBlockedPrinciple[id].id;
      this.$store.commit("setParamPage", param);
      this.$router.push({ name: "OP_BlockedPrincipleForm" });
    },
    doDeleteClick(ev, id) {
      console.log(ev, id)
    },
    ...
    getHeader() {
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/management-detail/header/sales-lead', param).then((response) => {
        ...
        this.ModelBlockedPrinciple = [];
        for (let i = 0; i < data.length; i++) {
          this.ModelBlockedPrinciple.push({
            ...data[i],
            propList: {
              url: "/api/hoonian-website/dashboard/operation-detail/blocking/principle/grid",
    ...
    renderList() {
      for (let i = 0; i < this.ModelBlockedPrinciple.length; i++) {
        this.ModelBlockedPrinciple[i].propList.ParamWhere = this.search;
        this.$refs['ref_blocked_principle'+i][0].doGetList("");
      }
    }
  },
  mounted() {
