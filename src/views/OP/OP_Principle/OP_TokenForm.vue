        ...
        <b-col lg="12" xl="12">
            <div class="card">
                ...
                <b-row class="title-primary">
                    ...
                    <span style="font-size: 18px; color: #4a93b3;">Token Refund</span>
                    </b-col>
                ...
                <div class="card__body">
                    ...
                    <b-row>
                        ...
                        <b-col style="border: solid 1px #dfe3f3; border-radius: 6px; padding: 15px !important; text-align: center;">
                            ...
                            <b-row style="margin-top: 20px;" class="poppins">
                                ...
                                <b-col lg="7" xl="7" style="margin-right: 10px; border: solid 1px #dfe3f3; border-radius: 6px; padding: 10px !important; text-align: center;">
                                    <div class="progress-x">
                                        <span>Token Value</span>
                                    </div>
                                    <div style="margin-top: 20px;">
                                        <span class="label" style="font-size: 30px;">IDR {{checkNum((Model.token_value && Model.token_value !== '' ? Model.token_value : 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','), (Model.token_value && Model.token_value !== '' ? Model.token_value : 0))}}</span>
                                    </div>
                                    <hr>
                                    <span style="font-size: 15px;" class="label"> IDR {{checkNum((Model.token_value/Model.token).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','), (Model.token_value/Model.token))}} / Token</span>
                                </b-col>
                            </b-row>
                        </b-col>
                    </b-row>
                    <b-row style="margin-top: 5px;">
                      <b-col md="5" offset-md="2">
                        <span>
                          <label class="lbl-bold poppins"> Refund Date </label>
                        </span>
                        <HOODateTime
                    ...
                    <b-row>
                      <b-col md="5" offset-md="2">
                        <span>
                          <label class="lbl-bold poppins"> Refund Token </label>
                        </span>
                        <ACCTextBox
                          :prop="PI_refund_token"
                          v-model="Model.refund_token"
                          ref="ref_refund_token"
                        />
                      </b-col>
                      <b-col md="5">
                          <span>
                          <label class="lbl-bold poppins"> Refund Amount </label>
                        </span>
                        <ACCTextBox
                          :prop="PI_refund_amount"
                          v-model="Model.refund_amount"
                          ref="ref_refund_amount"
                        />
                      </b-col>
                    </b-row>
                    <b-row>
  ...
  methods: {
    ...
    GetDataBy() {
        let param = {
            ...
            principle_id: this.paramFromList.id
        }
      this.postJSON(
        this.urlHoonian + "/api/hoonian-website/dashboard/operation-detail/token/detail",
        param
      ).then((response) => {
        ...
        this.Model.reference_no = "";
        this.Model.refund_token = "";
        this.Model.refund_amount = "IDR " + checkNum((this.Model.token_value && this.Model.token_value !== '' ? this.Model.token_value : 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','), (this.Model.token_value && this.Model.token_value !== '' ? this.Model.token_value : 0));
        this.Model.remarks = "";
      });
    },
    doSave() {
    ...
    M_Update() {
      let param = {
        ...
        project_id: this.paramFromList.project_id,
        refund_date: this.momentDateToUnix(this.Model.refund_date, "YYYY-MM-DD"),
        reference_no: this.Model.reference_no,
        refund_token: this.Model.refund_token,
        remarks: this.Model.remarks
      }

      this.postJSON(
        this.urlHoonian + "/api/hoonian-website/dashboard/operation-detail/token/refund",
        param
      ).then((response) => {
        if (response == null) return;
        this.doBack();
