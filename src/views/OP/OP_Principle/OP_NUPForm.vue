                            ...
                            <b-row style="margin-top: 20px;" class="poppins">
                                ...
                                <b-col lg="7" xl="7" style="margin-right: 10px; border: solid 1px #dfe3f3; border-radius: 6px; padding: 10px !important; text-align: center;">
                                    ...
                                    <div style="margin-top: 20px;">
                                        <span class="label" style="font-size: 30px;">IDR {{checkNum((Model.nup_value && Model.nup_value !== '' ? Model.nup_value : 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','), (Model.nup_value && Model.nup_value !== '' ? Model.nup_value : 0))}}</span>
                                    </div>
                                    <hr>
                                    <span style="font-size: 15px;" class="label"> IDR {{checkNum((Model.nup_value/Model.total_nup).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','), (Model.nup_value/Model.total_nup))}} / NUP</span>
                                </b-col>
                            </b-row>
                        </b-col>
                    </b-row>
...
export default {
  ...
  methods: {
    ...
    M_Update() {
      ...
      this.postJSON(
        this.urlHoonian + "/api/hoonian-website/dashboard/operation-detail/nup/refund",
        param
      ).then((response) => {
        if (response == null) return;
        this.doBack();
