    ...
    <div class="dashboard-page-chart__body">
      <b-row class="dashboardBody">
        <b-col lg="12" xl="12">
          ...
          <template v-for="(data,index) in ModelBlockedAgent">
...
export default {
  ...
  methods: {
    ...
    rowClicked(ev, id) {
      console.log(ev, id)
      let param = ev;
      param.project_id = this.ModelBlockedAgent[id].id;
      this.$store.commit("setParamPage", param);
      this.$router.push({ name: "OP_BlockedAgentForm" });
    },
    doDeleteClick(ev, id) {
      console.log(ev, id)
    },
    ...
    getHeader() {
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/management-detail/header/sales-lead', param).then((response) => {
        ...
        this.ModelBlockedAgent = [];
        for (let i = 0; i < data.length; i++) {
          this.ModelBlockedAgent.push({
            ...data[i],
            propList: {
              url: "/api/hoonian-website/dashboard/operation-detail/blocking/marketing/grid",
    ...
    renderList() {
      for (let i = 0; i < this.ModelBlockedAgent.length; i++) {
        this.ModelBlockedAgent[i].propList.ParamWhere = this.search;
        this.$refs['ref_blocked_agent'+i][0].doGetList("");
      }
    }
  },
  mounted() {
