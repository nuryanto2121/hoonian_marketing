      ...
      <b-row class="dashboardBody">
        <b-col md="12">
          <HOOList
            ...
            ref="ref_list"
            urlAdd="OP_ProjectForm"
            :WithDeleteButton="true"
            @buttonViewClicked="doViewClick"
            :ButtonAddDisabled="false"
          >
            <template slot="project_type" slot-scope="data">
              <span>{{data.item.project_type == 'L' ? 'Landed' : 'Highrise'}}</span>
            </template>
          </HOOList>
        </b-col>
      </b-row>
...
export default {
  data() {
    return {
      propList: {
        url: '/api/hoonian-website/project-master-list',
        initialWhere: '',
        SortField: '',
        SortBy: '',
        ParamWhere: '',
  ...
  methods: {
    ...
    doViewClick(record, index) {
      ...
      this.$router.push({ name: "OP_ProjectView" });
    },
