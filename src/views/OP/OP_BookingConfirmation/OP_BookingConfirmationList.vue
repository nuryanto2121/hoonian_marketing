      ...
      <b-row class="dashboardBody">
        <b-col md="12">
          <HOOList
            ...
            ref="ref_list"
            urlAdd="OP_BookingConfirmationForm"
            :WithDeleteButton="true"
            @buttonViewClicked="doViewClick"
            :ButtonAddDisabled="true"
          >
            <template slot="booked_date" slot-scope="data">
              <span>
                {{ momentUnix(data.item.booked_date, "YYYY-MM-DD") }}
              </span>
            </template>
          </HOOList>
        </b-col>
...
export default {
  data() {
    return {
      propList: {
        url: "/api/hoonian-website/booking-confirmation-list",
        initialWhere: "",
        SortField: "",
        SortBy: "",
        ParamWhere: "",
  ...
  methods: {
    ...
    doViewClick(record, index) {
      ...
      this.$router.push({ name: "OP_BookingConfirmationForm" });
    },
