    ...
    <div class="dashboard-page-chart__body">
      <b-row class="dashboardBody">
        <b-col lg="12" xl="12">
          ...
          <template v-for="(data,index) in ModelReservation">
                ...
                >
                  ...
                  <template slot="ToolbarTable">
                    ...
                    <b-col md="1" lg="1" xl="1" align-self="center" class="font-poppins noPadding" style="margin-right: 10px;">
                      <div align-self="center" style="--bgcolor: #52A0FC;" class="badge-header-list">
                    ...
                    <b-col md="1" lg="1" xl="1" align-self="center" class="font-poppins noPadding">
                      <div align-self="center" style="--bgcolor: #52A0FC;" class="badge-header-list">
                        <span style="margin: auto; font-size: 14px;">
                          {{checkNum(data.total_value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','), data.total_value)}}
                          {{checkPref(data.total_value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))}}
                        </span>
                      </div>
                    </b-col>
                  </template>
...
export default {
  ...
  methods: {
    ...
    getHeader() {
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/management-detail/reservation/header/marked-sold', param).then((response) => {
        if (response == null) return;
        let data = response.data;
        this.ModelReservation = [];
        for (let i = 0; i < data.length; i++) {
          this.ModelReservation.push({
            ...data[i],
            total_unit: data[i].total_unit && data[i].total_unit !== '' ? data[i].total_unit : 0,
            total_value: data[i].total_value && data[i].total_value !== '' ? data[i].total_value : 0,
            propList: {
              url: "/api/hoonian-website/dashboard/management-detail/reservation/grid/marked-sold",
    ...
    renderList() {
      for (let i = 0; i < this.ModelReservation.length; i++) {
        this.ModelReservation[i].propList.ParamWhere = this.search;
