                        ...
                        <template slot="ToolbarTable">
                          ...
                          <b-col md="1" lg="1" xl="1" align-self="center" class="font-poppins noPadding" style="margin-right: 10px;">
                            <div align-self="center" style="--bgcolor: rgb(255 199 0);" class="badge-header-list">
                              <span style="margin: auto; font-size: 14px;">
                                {{checkNum(data.sum.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','), data.sum)}}
                                {{checkPref(data.sum.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','))}}
                              </span>
                            </div>
                          </b-col>
                          <b-col style="max-width: max-content;" align-self="center" class="col-right font-poppins lbl-header-list">
...
export default {
  ...
  methods: {
    ...
    getProject() {
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/management-detail/header/payment-method', param).then((response) => {
        ...
        for (let i = 0; i < data.length; i++) {
          this.ModelProject.push({
            ...
            propList: {
              ...
              initialWhere: this.Model.project_id,
              SortField: "",
              SortBy: "desc",
              ParamWhere: "",
              param: {
                search: this.search,
                payment_method_id: data[i].payment_method_id
              }
            }
          });
        }
    ...
    renderList(id = -1) {
      if (id < 0) {
        for (let i = 0; i < this.ModelProject.length; i++) {
          // console.log('ref_payment_method_'+i, this.$refs['ref_payment_method_'+i])
          this.$refs['ref_payment_method_'+i][0].doGetList("");
        }
      }
      else {
        ...
        this.$refs['ref_payment_method_'+id][0].doGetList("");
      }
    }
  },
  mounted() {
