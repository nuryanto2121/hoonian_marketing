    ...
    <div class="dashboard-page-chart__body">
      <b-row class="dashboardBody">
        <b-col lg="12" xl="12">
          ...
          <template v-for="(data,index) in ModelSalesEvent">
...
export default {
  ...
  methods: {
    ...
    getHeader() {
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/management-detail/header/sales-event', param).then((response) => {
        if (response == null) return;
        let data = response.data;
        this.ModelSalesEvent = [];
        for (let i = 0; i < data.length; i++) {
          this.ModelSalesEvent.push({
            ...data[i],
            label: data[i].project_name,
            propList: {
              url: "/api/hoonian-website/dashboard/management-detail/grid/sales-event",
    ...
    renderList() {
      for (let i = 0; i < this.ModelSalesEvent.length; i++) {
        this.ModelSalesEvent[i].propList.ParamWhere = this.search;
        this.$refs['ref_sales_event_'+i][0].doGetList("");
      }
    }
  },
  mounted() {
