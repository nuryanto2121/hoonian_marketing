    ...
    <div class="dashboard-page-chart__body">
      <b-row class="dashboardBody">
        <b-col lg="12" xl="12">
          ...
          <template v-for="(data,index) in ModelInternalMarketing">
            <b-row v-bind:key="index">
              <b-col lg="12" xl="12">
                <HOOList
                  ...
                  :ref="'ref_internal_marketing_'+index"
                ...
                >
                  ...
                  <template slot="join_date" slot-scope="data">
                    {{ momentUnix(data.item.join_date, "DD MMM YYYY") }}
                  </template>
                  <template slot="sales_value" slot-scope="data">
                    {{isCurrency(data.item.sales_value, decimal)}}
                  </template>
                  <template slot="total_sold_unit" slot-scope="data">
                    {{isCurrency(data.item.total_sold_unit, 0)}}
                  </template>
                </HOOList>
              </b-col>
            </b-row>
...
export default {
  ...
  methods: {
    ...
    getHeader() {
      ...
      this.postJSON(this.urlHoonian + '/api/hoonian-website/dashboard/all-projects-assign', param).then((response) => {
        if (response == null) return;
        let data = response.data;
        this.ModelInternalMarketing = [];
        for (let i = 0; i < data.length; i++) {
          this.ModelInternalMarketing.push({
            ...data[i],
            propList: {
              url: "/api/hoonian-website/dashboard/management-detail/grid/internal-marketing",
    ...
    renderList() {
      for (let i = 0; i < this.ModelInternalMarketing.length; i++) {
        this.ModelInternalMarketing[i].propList.ParamWhere = this.search;
        this.$refs['ref_internal_marketing_'+i][0].doGetList("");
      }
    }
  },
  mounted() {
