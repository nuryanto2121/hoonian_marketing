                  ...
                  <b-col md="10">
                    <b-row>
                      <b-col md="6">
                        ...
                        <ACCTextBox
                          :prop="PI_payment_method_name"
                          v-model="Model.payment_method_name"
                          ref="ref_payment_method_name"
                        />
                      </b-col>
                      <b-col
...
export default {
  data() {
    return {
      ...
      PI_payment_method_name: {
        cValidate: "required|max:100",
        cName: "payment_method_name name",
        cOrder: 1,
        cKey: false,
        cType: "text",
        cProtect: false,
  ...
  methods: {
    ...
    M_Save() {
      this.postJSON(
        this.urlHoonian + "/api/hoonian-website/payment-method-master-addedit",
        this.Model
      ).then((response) => {
    ...
    M_Delete() {},
    GetDataBy() {
      this.Model.payment_method_master_id = this.paramFromList.id;
      this.postJSON(
        this.urlHoonian + "/api/hoonian-website/payment-method-master-detail",
        this.Model
      ).then((response) => {
        if (response == null) return;
        this.Model = response.data.payment_method_master_detail;
        this.Model.payment_method_master_id = this.Model.id;
