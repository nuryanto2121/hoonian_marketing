            ...
            <div class="card__title">
              <b-row>
                <b-col style="max-width: max-content !important">
                  <span>User Management Internal Access Button</span>
                </b-col>
              ...
              <b-row>
                <b-col>
                  <ACCFormList
                    ...
                    ref="ref_UserManagementButtonAccess"
                  >
...
export default {
  ...
  methods: {
    onAccessCheck(index, checked, displayButton) {
      if (displayButton.toLowerCase() == 'view') {
        for (let x = index; x < this.$refs.ref_UserManagementButtonAccess.items.length; x++ ) {
          if (this.$refs.ref_UserManagementButtonAccess.items[index].display_form != this.$refs.ref_UserManagementButtonAccess.items[x].display_form) {
            break;
          }
          this.$refs.ref_UserManagementButtonAccess.items[x].is_access = checked;
          if (x != index)
            this.$refs.ref_UserManagementButtonAccess.items[x].is_disabled = !checked;
        }
      }
    },
    doBack() {
    ...
    M_Save() {
      let items = this.$refs.ref_UserManagementButtonAccess.items;
      var param_Delete = {
          _Method_: "DELETE",
          _LineNo_: 1,
          menu_url: this.paramFromList.ButtonAccess.menu_url,
          ss_portfolio_id: this.paramFromList.ss_portfolio_id,
          user_id: this.paramFromList.ButtonAccess.user_id,
        },
        param_Save = [];
  ...
  mounted() {
    // this.propList.initialWhere = "ss_portfolio_id='" + this.getDataUser().portfolio_id + "' AND ss_group_id='" + this.paramFromList.ButtonAccess.ss_group_id + "' AND menu_url='" + this.paramFromList.ButtonAccess.menu_url + "'"
    this.propList.ParamView =
      "'" +
      this.paramFromList.ss_portfolio_id +
      "','" +
      this.paramFromList.ButtonAccess.user_id +
      "','" +
      this.paramFromList.ButtonAccess.menu_url +
      "'";
    this.$refs.ref_UserManagementButtonAccess.doGetList("");
  },
};
</script>
