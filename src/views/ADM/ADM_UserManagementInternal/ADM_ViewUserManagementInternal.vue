...
export default {
  ...
  methods: {
    addPortfolio() {
      ...
      param.isEdit = true;
      this.$router.push({ name: "ADM_AddPortfolioUserManagementInternal", params: param });
    },
    doPortfolio(record) {
      let param = record;
      param.row_id = this.paramFromList.row_id;
      param.user_id = this.paramFromList.user_id;
      param.group_access = this.paramFromList.group_access;

      this.$store.commit("setParamPage2", param);
      this.$router.push({ name: "ADM_DetailUserManagementInternal" });
    },
    ...
    getUserInternal() {
      let param = {
        option_function_cd: "GetUserInternal",
        module_cd: "SS",
      ...
      this.CallFunction(param).then((response) => {
        if (response == null) return;
        let data = JSON.parse(response.Data[0].fss_user_internal_s)[0];
