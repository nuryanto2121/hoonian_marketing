<template>
  <div v-if="Model.length > 0" style="height: 95%; background: #F8F8F8; margin-top: 20px;">
    <b-row>
      ...
      <b-col style="font-size: 14px; text-align: right; padding-right: 12px !important; color: #4A93B3; cursor: pointer;">
        <span @click="viewAll">
          {{ $t('view_all') }}
        </span>
      </b-col>
        ...
        "  @click="doViewDetail(data)">
          <b-row>
            <b-col style="height: 200px; padding: 0px !important;">
              <b-img :src="urlHoonian + data.picture" alt="" class="center" style="max-height: 100%; width: 100px;" fluid-grow @error="onImageLoadFailure($event)" />
