<template>
  <span>
    ...
    <b-col md="1" style="text-align: center !important;">
        <!-- <span>Time</span> -->
        <template v-if="languageStatus">
        ...
        <template v-else>
          <label v-show="inputStatus=='new' || inputStatus=='edit'" class="text-field-form"> - </label>
          <label v-show="inputStatus=='view'" class="lbl-view-form"> - </label>
        </template>
    </b-col>
    ...
    <b-col>
        <masked-input
        :placeholder="prop.cFormat"
        v-validate="'invalidHour|invalidTime|invalidTimeRange:' + valueFrom"
        v-model="valueTo"
        v-show="inputStatus!=='view'"
        v-bind:name="prop.cName + 'TimeTo'"
        v-bind:data-vv-as="prop.cLabel + 'TimeTo'"
        :disabled="inputStatus=='edit' && prop.cKey || prop.cProtect"
        :tabindex="prop.cOrder + 1"
        mask="11:11"
        class="text-field-form form-control"
        :class="{'input': true, 'error-text-field': errors.has(prop.cParentForm + '.' + prop.cName + 'TimeTo')}"
        @blur.native="timeTo" />
        <!-- @blur.native="addTime" -->
        
        <span v-show="errors.has(prop.cParentForm+'.'+prop.cName + 'TimeTo')"
          class="error-span">{{ errors.first(prop.cParentForm+'.'+prop.cName + 'TimeTo') }}
        </span>

        <label class="lbl-value-view-form notranslate" v-show="inputStatus=='view'">{{valueTo}}</label>
    </b-col>
...
export default {
  ...
  computed: {
    ...
    // valueLabelTimeFrom() {
    //   return this.valueFrom?moment(new Date(this.valueFrom)).format('HH:mm') : ''
    // },

    // valueLabelTimeTo() {
    //   return this.valueTo?moment(new Date(this.valueTo)).format('HH:mm') : ''
    // },
  ...
  methods: {
    ...
    timeTo () {
      this.$emit('to', this.valueTo == null ? '' : this.valueTo)
    },
    handleInput() {
  ...
  created: function() {
    ...
    this.$validator.extend('invalidTimeRange', {
      getMessage(field, val) {
        return 'End time must be higher than start time'
      },
      validate(value, field) {
        // let startParts = self.valueFrom.split('-')

        let startParts = field[0].split(':')
        let endParts = value.split(':')
        
        let start = startParts[0] + startParts[1]
        let end = endParts[0] + endParts[1]
        return end >= start
      }
    })
